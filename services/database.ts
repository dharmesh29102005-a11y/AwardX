...
class DatabaseService {
  ...
  async getJudgeScoresByRound(programId: string) {
    ...
    for (const row of assignmentRows || []) {
      const rawJudge = row.judges;
      const judgeObj = Array.isArray(rawJudge) ? rawJudge[0] : rawJudge;
      const judge = judgeObj as { id: string; name: string; email: string } | null;

      const rawSub = row.submissions;
      const subObj = Array.isArray(rawSub) ? rawSub[0] : rawSub;
      const submission = subObj as { id: string; title: string; applicant_name?: string } | null;

      const rawRound = row.rounds;
      const roundObj = Array.isArray(rawRound) ? rawRound[0] : rawRound;
      const round = roundObj as { id: string; title: string; type: string; status: string } | null;
